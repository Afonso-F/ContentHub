...
async function gerarAfiliadoDeConceito() {
  ...
  await _runConceptGen('afiliado', `Cria um perfil de programa de afiliados para criador de conteúdo baseado nesta descrição: "${conceito}"
...
Responde APENAS com JSON válido, sem markdown, sem backticks:
{
  ...
  "comissao_pct": 10
    ...
    _setField('af-nome',       data.nome);
    _setField('af-plataforma', data.plataforma || '');
    if (data.comissao_pct) _setField('af-comissao', data.comissao_pct);
    return `Programa "${data.nome}" gerado!`;
...
async function openAfiliadoModal(id) {
  ...
  const body = `
    <div class="concept-toolbar" style="margin-bottom:0">
      <button id="concept-toggle-btn-afiliado" class="btn btn-sm btn-ghost" onclick="_toggleConceptBar('afiliado')">
