...
async function gerarPromptDeConceito() {
  ...
  await _runConceptGen('bib', `Cria uma entrada de biblioteca de prompts para geração de imagens com IA baseada nesta ideia: "${conceito}"
...
Responde APENAS com JSON válido, sem markdown, sem backticks:
{
  ...
  "tags": ["tag1", "tag2", "tag3"]
    ...
    _setField('bib-f-titulo', data.titulo);
    _setField('bib-f-prompt', data.prompt || '');
    if (data.tags && Array.isArray(data.tags)) _setField('bib-f-tags', data.tags.join(', '));
    if (data.tipo)     _setField('bib-f-tipo', data.tipo);
    if (data.categoria) _setField('bib-f-cat', data.categoria);
    return `Prompt "${data.titulo}" gerado!`;
...
function openAddPromptModal(existing) {
  const body = `
    <div class="concept-toolbar" style="margin-bottom:0">
      <button id="concept-toggle-btn-bib" class="btn btn-sm btn-ghost" onclick="_toggleConceptBar('bib')">
        <i class="fa-solid fa-wand-magic-sparkles"></i> Descrever com IA
      </button>
    </div>
    <div class="concept-panel" id="concept-panel-bib">
